...
function ItemsViewModel() {
    ...
    function productModel(data) {
        ...
        var description = data.description().length > 25 && $('#adminView').length === 0 ? data.description().substring(0, 25) + " ..." : data.description();
        var categoryName = data.categoryName() === null ? "UnCategorized" : data.categoryName();
    ...
    self.jsonToArray = function (json) {
        var result = [];
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.message) {
                    ...
                    var filteredItems = new productModel(response);
                    self.filteredItems.push(filteredItems);
    ...
    self.ccountry.subscribe(function (value) {
        if (value) {
            var provinces = _.filter(self.bigData()[0], function (item) {
                return item.country === value;
            });
            if (provinces.length !== 0) {
                self.provinces(provinces[0].states);
            }
        } else {
            self.provinces([]);
        }
